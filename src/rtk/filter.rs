...
//!
//! Vendored from rtk-ai/rtk v0.35.0 (`src/core/filter.rs`).
//! Copyright 2024 Patrick Szymkowiak — Apache License 2.0.
//!
//! Local modifications:
//! - Replaced `lazy_static!` with `std::sync::LazyLock` (stable since 1.80)
//!   to avoid adding `lazy_static` to seshat's dependency set.
...
static MULTIPLE_BLANK_LINES: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"\n{3,}").expect("valid regex: multiple blank lines"));
...
static IMPORT_PATTERN: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"^(use |import |from |require\(|#include)").expect("valid regex: import pattern")
});
static FUNC_SIGNATURE: LazyLock<Regex> = LazyLock::new(|| {
    ...
    .expect("valid regex: function signature")
});
...
mod tests {
    ...
    fn test_json_no_comment_stripping() {
        // Reproduces rtk #464: package.json with "packages/*" was corrupted
