...
export default function BenchPage() {
	return (
		<main className="bench-page-fx relative min-h-dvh overflow-x-hidden overflow-y-auto text-[var(--text)]">
			<header
				className="sticky top-0 z-20 flex items-center justify-between gap-6 border-b border-(--line) bg-[var(--topbar-bg)] px-[clamp(18px,5vw,64px)] py-4 backdrop-blur-[18px] supports-[backdrop-filter]:backdrop-blur-xl max-[900px]:flex-col max-[900px]:items-start"
			...
			<section
				className="bench-hero-fx relative mx-auto mt-[clamp(24px,4vw,46px)] grid min-h-[clamp(470px,64svh,610px)] w-[calc(100%-48px)] max-w-[1180px] grid-cols-[minmax(0,0.95fr)_minmax(360px,1.05fr)] items-center gap-[clamp(30px,5vw,76px)] overflow-hidden rounded-lg border border-(--line) p-[clamp(34px,6vw,68px)] shadow-[0_42px_120px_rgba(0,0,0,0.18),inset_0_1px_0_rgba(242,245,239,0.08)] backdrop-blur-xl max-[900px]:grid-cols-1 max-[540px]:w-[calc(100%-32px)] max-[540px]:p-4"
			...
			>
				<div className="relative z-[1] min-w-0">
					...
					<div className="mt-8 flex flex-wrap gap-2.5" aria-label="Parametros do bench">
						{["codex", "claude", "ollama"].map((agent) => (
							<span
								className="inline-flex min-h-8 items-center rounded-full border border-(--line) bg-[var(--button-secondary-bg)] px-3 text-[0.82rem] font-extrabold text-[var(--text)]"
								key={agent}
							>
								{agent}
							</span>
				...
				<div className="relative z-[1] grid">
					<pre className="m-0 min-w-0 overflow-x-auto whitespace-pre-wrap rounded-lg border border-(--line) bg-[var(--bench-code-bg)] px-[22px] py-5 font-mono text-[0.84rem] leading-[1.56] text-[var(--pre-text)] shadow-[0_22px_70px_rgba(0,0,0,0.14)] [overflow-wrap:anywhere] [word-break:break-all] max-[540px]:p-4 max-[540px]:text-[0.78rem]">{`seshat bench agents \\
