...
class ToolingRunner:
    ...
    def _discover_typescript_tools(self) -> ToolingConfig:
        ...
        if "typescript" in deps:
            ...
            self._apply_command_overrides(
                tool, self.seshat_config.checks.get("typecheck", {})
            )
        ...
        if "jest" in deps:
            ...
            if "test" in scripts:
                ...
                tool.pass_files = False
            self._apply_command_overrides(
                tool, self.seshat_config.checks.get("test", {})
            )
    ...
    def _get_tool_config(self, tool_name: str, check_type: str) -> ToolCommand:
        ...
        self._apply_command_overrides(tool, check_config)
        
        return tool

    def _get_command_config(self, tool: ToolCommand) -> dict:
        """Get command configuration from .seshat for the tool or check type."""
        commands = self.seshat_config.commands
    ...
    def _apply_command_overrides(self, tool: ToolCommand, check_config: dict) -> None:
        """Apply command, extensions, and file passing overrides."""
        command_config = self._get_command_config(tool)
        ...
        if tool.check_type == "lint" and self.detect_project_type() == "typescript":
